...
```
src/
  engine/        nimRules.ts / nimAI.ts / xorCalculator.ts
  store/         gameStore.ts / recordStore.ts / challengeStore.ts
  components/    GameBoard / EnergyPile / XORCore / BinaryPanel / ControlPanel / ParticleLayer / GameOverModal
  views/         Home / Game / Tutorial / Challenge / Records / Settings
  sound/         sfx.ts（WebAudio 合成）
